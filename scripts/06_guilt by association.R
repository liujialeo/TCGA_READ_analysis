...
rm(list = ls())
dir.create("output/FFAR2_associated")
### 加载表达数据，这是vst标准化后的数据
...
save(correlation, file = "./output/FFAR2_association.Rdata")
## 排序
library(dplyr)
...
ggsave("output/FFAR2_associated/GO_enrichment.pdf")
#################################
## GSEA
...
ggplot(y, showCategory = 34, aes(NES, forcats::fct_reorder(Description, NES))) + 
